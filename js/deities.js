...
class DeitiesPage extends ListPage {
	...
	getListItem (g, dtI, isExcluded) {
		...
		const alignment = g.alignment ? g.alignment.map(a => Parser.alignmentAbvToFull(a)).join("") : "\u2014";
		const domains = g.domains.map(d => Parser.SubclassToDisplay(d)).join(", ");

		eleLi.innerHTML = `<a href="#${hash}" class="lst--border lst__row-inner">
			<span class="bold col-3 pl-0">${g.name}</span>
			<span class="col-2 text-center">${Parser.PantheonToDisplay(g.pantheon)}</span>
