...
class HashLoad {
	static loadhash (id) {
		...
		function getSkillProfString (skills) {
			...
			return skills.from.length === 18 ? `選擇任意${numString}個技能。` : `從 ${skills.from.map(it => EntryRenderer.getDefaultRenderer().renderEntry(`{@skill ${it}}`)).joinConjunct(", ", " and ")} 中選擇${numString}個。`
		}
